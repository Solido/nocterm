  ...
  void _startUpdates() {
    _updateTimer = Timer.periodic(Duration(milliseconds: 100), (_) {
      ...
      setState(() {
        ...
        _cpuPercent = (_cpuPercent + (_random.nextDouble() - 0.5) * 10)
            .clamp(10.0, 95.0);
        _memoryGb =
            (_memoryGb + (_random.nextDouble() - 0.5) * 0.2).clamp(1.0, 8.0);
        _requestsPerSec =
            (_requestsPerSec + (_random.nextDouble() - 0.5) * 200)
                .clamp(500.0, 3000.0);
        _activeConnections =
        ...
        for (var task in _tasks) {
          if (task.progress < 1.0) {
            task.progress = (task.progress + _random.nextDouble() * 0.02)
                .clamp(0.0, 1.0);
          } else {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildMetric('MEM', '${_memoryGb.toStringAsFixed(1)}GB',
              Color(0xFFa371f7)),
          _buildMetric('REQ', '${(_requestsPerSec / 1000).toStringAsFixed(1)}k/s',
              Color(0xFF58a6ff)),
