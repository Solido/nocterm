...
void main() {
  group('Checkmark Unicode Width Issue', () {
    test('checkmark (✓) vs checkmark button (✅) width', () {
      ...
      final checkmarkButton = '✅'; // U+2705 - Check mark button (emoji, 2 width)

      print('Checkmark ✓:');
      print('  Code: U+${checkmark.runes.first.toRadixString(16).toUpperCase()}');
      print('  Current width: ${UnicodeWidth.stringWidth(checkmark)}');
      print('  Expected width: 1');

      print('\nCheckmark Button ✅:');
      print('  Code: U+${checkmarkButton.runes.first.toRadixString(16).toUpperCase()}');
    ...
    test('other common checkmark-like symbols', () {
      ...
      symbols.forEach((symbol, expectedWidth) {
        ...
        print('$symbol (U+${symbol.runes.first.toRadixString(16).toUpperCase()}): '
              'actual=$actual, expected=$expectedWidth');

        expect(
          actual,
          equals(expectedWidth),
          reason: 'Symbol $symbol should have width $expectedWidth but has $actual',
        );
      });
    });
